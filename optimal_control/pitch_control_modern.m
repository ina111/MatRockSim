...
%A_body = [-L_alpha/mass/vel_A -g*cosd(theta0) vel_A;
%	 0 0 1;
%	 L_alpha*length_A/Iyy/vel_A 0 0];
%B_body = [force_T/mass; 0; force_T*length_T/Iyy];
%Co_body = ctrb(A_body,B_body);
%rank(Co_body);
%
%A_motor = [0 1 0;
%	 0 0 KT/(Jm+JL/gear);
%	 0 -KE/L -R/L];
%B_motor = [0; 0; 1/L];
%Co_motor = ctrb(A_motor,B_motor);
%rank(Co_motor);


% ----状態方程式----
%　sys: 状態方程式
% x = [z_dot; θ; θ_dot; delta], u = delta_i, y = [θ; delta]
% z_dot: 機軸垂直方向速度
% θ: 目標角とのズレ
% delta: ジンバル角
% delta_i: ジンバル角入力値
...
% sys_servo: 拡大系
% ----------------
A_reg = [-L_alpha/mass/vel_A g*cosd(theta0) -vel_A force_T/mass;
	 0 0 1 0;
	 L_alpha*length_A/Iyy/vel_A 0 0 -force_T*length_T/Iyy;
	 0 0 0 -1/tau_delta];
B_reg = [0; 0; 0; 1/tau_delta];
C_reg = A_reg(1:2,:);
...
As_upper = horzcat(A_reg, [0; 0; 0; 0]);
As_lower = [0 1 0 0 0];
...
legend('delta');;ylabel('deg');
subplot(2,1,2);plot(t,rad2deg(x0(:,5)));grid;
